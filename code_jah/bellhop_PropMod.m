...
settingsPath = 'F:\PropaMod\Settings.xlsx'; % <- Aaron
...
for rad = 1:length(radials)
    ...
    zssp = 1:5:max(bath)+100; % JAH hard code every 5 m
...
for freqi = 1:length(freq)
    ...
    thisAngle = radials; sd = SD;
    save(fnmatOut, 'thisAngle', 'rr', 'nrr', 'botDepthSort', 'sd', 'sortedTLVec', 'IX','rd_all')
