...
regionabrev = 'WAT';
GDrive = 'C';
FilePath = [GDrive ':\Users\HARP\Documents\AD_Working\hycom_temp\']; % FilePath = [GDrive ':\Users\HARP\Documents\AD_Working\hycom_temp\' regionabrev];
saveDir = 'C:\Users\HARP\Documents\AD_Working\hycom_tempOutput';
...
MonthEnd = '201812';    % Last month of study period (yyyymm)
...
hycom_sampleMonths('2015-07-01 00:00:00', '2019-06-01 00:00:00', 'C:\Users\HARP\Documents\AD_Working\hycom_temp', 'C:\Users\HARP\Documents\AD_Working\hycom_temp')
...
for b = 1:length(siteabrev)         % Generate subfolders for each site if they don't exist yet
    if exist(fullfile(saveDir, siteabrev(b,:)), 'dir')
    else
        mkdir(fullfile(saveDir, siteabrev(b,:)))
    end
end
...
for i = 1:length(siteabrev(:,1))
    ...
    writetable(SSPM, [saveDir,'\', siteabrev(i,:),'\', siteabrev(i,:), '_SSPMmin_',num2str(sprintf('%02d', minMo)),'.xlsx']) % Save minimum month average SSP
    disp(['Average maximum month SSP saved for ' Site '. At this site, Month ' num2str(sprintf('%02d', minMo)) ' has the slowest average SSP.'])
    ...
    SSPM = array2table(SSPM);
    SSPM.Properties.VariableNames = {'Depth' 'SS'};
    writetable(SSPM, [saveDir,'\', siteabrev(i,:),'\',siteabrev(i,:), '_SSPMmax_',num2str(sprintf('%02d', maxMo)),'.xlsx']) % Save maximum month average SSP
