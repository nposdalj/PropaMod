...
% Generate new .shd files with 3 digits
for k = 1:length(matFiles)
    oldFileName = matFiles(k,:);
    firsthalf = oldFileName(1:strfind(oldFileName,'_'));
    if matFiles(k,13) == ' '
        secondhalf = oldFileName(strfind(oldFileName,'.shd')-1:end);
        newFileName = strcat(firsthalf, '00', secondhalf);
    elseif matFiles(k,14) == ' '
        secondhalf = oldFileName(strfind(oldFileName,'.shd')-2:end);
...
disp(['Radial Map saved: Site ', SITE, ', Depth ', num2str(plotdepth), ' m'])
