...
userNote = ' WC, MidSize and Social Groups'; % Include a note for yourself/others. This will be included in the .txt output.
...
SL = 220; % Source Level
SD = NaN; % 800; % Source depth % <- Should this... just be the same as hdepth...?
% ^ Before this was 800 but now I am just setting it to be the depth at
% hlat and hlon, - 10 m. To simulate a hydrophone sitting 10 m off of the
% bottom. This calculation is made later in the code, after the bathymetry
% is generated by makeBTY (within the big loop). It is only calculated
% once, during the generation of the first radial.
hlat = 38.3738; % 39.8326; % hydrophone lat
hlon = -73.37; % -69.9800; % hydrophone long
hdepth = 990; % hydrophone depth % <- This doesn't appear to actually do anything but it's input into DetSim_Workspace
