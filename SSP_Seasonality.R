...
SSP_MoAvgs <- cbind(SSP_M01,SSP_M02,SSP_M03,SSP_M04,SSP_M05,SSP_M06,
                    SSP_M07,SSP_M08,SSP_M09,SSP_M10,SSP_M11,SSP_M12)
...
points(SSP_M07[depthlist_range],-depthlist[depthlist_range], 'l', col="#FF8000") # July - Orange
points(SSP_M08[depthlist_range],-depthlist[depthlist_range], 'l', col="#FFFF00")
points(SSP_M09[depthlist_range],-depthlist[depthlist_range], 'l', col="#80FF00")
points(SSP_M10[depthlist_range],-depthlist[depthlist_range], 'l', col="#00FF00") # October - Green
...
range800 <- range(M800)
min_mo800 <- which(M800 == range800[1])
max_mo800 <- which(M800 == range800[2])

sumtab800 <- cbind(depthlist[31],SSP_MAnnual[31],SSP_stdev[31],min_mo800,range800[1],max_mo800,range800[2])
colnames(sumtab800) <- c("Depth","c_mean","c_stdev","min_mo800","c_min","max_mo800","c_max")

#Calculate months where average sound speeds across depths 200m+ are max and min
MoAvgs_200plus <- colMeans(SSP_MoAvgs[23:40,], na.rm=TRUE)
range_MoAvg <- range(MoAvgs_200plus)
min_mo = which(MoAvgs_200plus == range_MoAvg[1])
max_mo = which(MoAvgs_200plus == range_MoAvg[2])
sumtab_extremes <- cbind(min_mo,max_mo)
