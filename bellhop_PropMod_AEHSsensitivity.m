...
compressional_speed = 1400:100:1600;   % Compressional speed
shear_speed = 140:10:160;   % Shear speed
density_aehs = 1.4:.3:2.0;        % Density
compressional_atten = 0:0.1:0.1;    % Compressional attenuation

% CONFIGURE OUTPUT RANGE AND RESOLUTION
total_range = 20000;    % Radial range around your site, in meters
rangeStep = 20;         % Range resolution % CHANGED FROM 10 M TO 20 M
depthStep = 20;         % Depth resolution % CHANGED FROM 10 M TO 20 M
...
if dailyFolderNum == 123    % This is the double value of {, which comes after z (double value 122)
    ...
    intermedDirF1 = intermedDir; % Local subdirectory for 1st freq
    ...
    saveDir_subF1 = saveDir_sub; % Save subdirectory for 1st freq
    ...
    plotDirF1 = plotDir; % Plot subdirectory for 1st freq
...
for rad = 10
    ...
    % at edge of circle
    lati(rad, 1:(nrr+1)) = linspace(hydLoc(1, 1), latout(rad), length(0:rangeStep:total_range));
    loni(rad, 1:(nrr+1)) = linspace(hydLoc(1, 2), lonout(rad), length(0:rangeStep:total_range));
    ...
    for freqi = 1:length(freq)
        ...
        filePrefix_general = ['R' radialiChar '_' freqiChar 'kHz_'];
        
        copyfile(fullfile(intermedDir,['R' radialiChar '.bty']),...
            fullfile(intermedDirFi, [filePrefix_general '.bty'])); % copy bty from intermed dir to intermed subdir
        copyfile(fullfile(intermedDirFi, [filePrefix_general '.bty']),...
            fullfile(saveDir_subFi, [filePrefix_general '.bty']));    % copy bty to final save dir
        
        for cS = 1:length(compressional_speed) % Begin cycle through compressional speeds
            for sS = 1:length(shear_speed) % Begin cycle through shear speeds
                for den = 1:length(density_aehs) % Begin cycle through densities
                    for cA = 1:length(compressional_atten) % Begin cycle through compressional attenuations
                        
                        AEHS.compSpeed = compressional_speed(cS);   % Compressional speed
                        AEHS.shearSpeed = shear_speed(sS);   % Shear speed
                        AEHS.density = density_aehs(den);        % Density
                        AEHS.compAtten = compressional_atten(cA);    % Compressional attenuation
                        AEHS.shearAtten = 0.0;   % Shear attenuation
                        
                        filePrefix = ['R' radialiChar '_' freqiChar 'kHz_' num2str(cS) num2str(sS) num2str(den) num2str(cA)];
                        
                        copyfile(fullfile(intermedDirFi, [filePrefix_general '.bty']),...
                            fullfile(intermedDirFi, [filePrefix '.bty']))
                    ...
                    end % End cycl through compressional attenuations
                end % End cycle through densities
            end % End cycle through shear speeds
        end % End cycle through compressional speeds
    end
