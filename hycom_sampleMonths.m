function hycom_sampleMonths(startMonth, endMonth, local_outpath, final_outpath)
...
% excluded.
%
% Note that the script is currently configured for the Western Atlantic Ocean.
...
for itr = 1:3  % Loop through iterations
    ...
    for i = 1:nt % Loop through time points
        ...
        elseif ((datenum(2019,1,1) >= stdate) || (stdate <= eddate))
            ...
            xl = [3476:3714];yl = [2600:3100];zl = [0:1:39];    % GLBy0.08 has a different GRID!
            ext_hycom_gofs_93_0(opath,f1,str_date,zl,yl,xl,OpenDAP_URL);
        end
